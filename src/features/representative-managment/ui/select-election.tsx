...
export function SelectElection({ value, onChange, options, label }: Props) {
  return (
    <div className="flex flex-col">
      ...
      <select
        className="relative z-10 p-4 font-light bg-transparent border-2 outline-none transition disabled:opacity-70 disabled:cursor-not-allowed block w-full rounded-md bg-gray border-gray-400 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm placeholder-gray-500 my-2 text-gray-900 form__select-bootcamp"
        name="election"
        id="election"
        value={value}
